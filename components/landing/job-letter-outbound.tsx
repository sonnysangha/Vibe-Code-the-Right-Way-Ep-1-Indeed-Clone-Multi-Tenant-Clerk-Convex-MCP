...
const SPREAD_BY_INDEX = [1.35, -1.05, 0.75, -1.4, 1.15, -0.9];
const ARC_LIFT_PX = -26;
...
function controlPoints(
  ...
  const nx = -dy / len;
  const ny = dx / len;
  const bend = Math.min(140, len * 0.42) * spread;
  const liftY = lift * spread;
  const midX = (start.x + end.x) / 2;
  const midY = (start.y + end.y) / 2 + liftY;
  return [
    { x: midX + nx * bend, y: midY + ny * bend * 1.05 },
    { x: midX - nx * bend * 0.42, y: midY - ny * bend * 0.5 + liftY * 0.35 },
  ];
}

/** Aim slightly above wordmark centers so arcs feel like they “dock” above the logos. */
function approachTarget(fullEnd: Point, start: Point): Point {
  const dx = fullEnd.x - start.x;
  const dy = fullEnd.y - start.y;
  const len = Math.max(80, Math.hypot(dx, dy));
  const ux = dx / len;
  const uy = dy / len;
  const shorten = Math.min(36, len * 0.08);
  return {
    x: fullEnd.x - ux * shorten,
    y: fullEnd.y - uy * shorten - 6,
  };
}
...
export function JobLetterOutbound({
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 mt-10 flex flex-wrap items-baseline justify-center gap-x-10 gap-y-6 md:mt-14 md:justify-between md:gap-x-6 md:gap-y-8">
        {companies.map((company, i) => (
          <span
            ...
            className={cn(
              "relative isolate select-none px-1 text-muted-foreground/55 transition-colors duration-300 hover:text-muted-foreground/80",
              company.className,
            )}
          >
            {company.name}
      ...
      <p className="relative z-30 mx-auto mt-8 max-w-lg pb-8 text-center text-sm text-muted-foreground md:pb-10">
      ...
      {geometry && n > 0 && (
        <svg
          ...
          aria-hidden
        >
          {geometry.targets.slice(0, n).map((fullEnd, i) => {
            const end = approachTarget(fullEnd, geometry.start);
            const spread = SPREAD_BY_INDEX[i % SPREAD_BY_INDEX.length];
            ...
            const pathLenEstimate =
              ...
              1.42;
            const dashOffset = pathLenEstimate * (1 - eased * 0.9);
      ...
      {geometry &&
        ...
        geometry.targets.slice(0, n).map((fullEnd, i) => {
          ...
          const scale = 0.5 + eased * 0.38;
          const rotate = (1 - eased) * (-7 + i * 2);
          const exitStart = 0.56;
          const arriveFade =
            eased <= exitStart
              ? 1
              : Math.max(0, 1 - (eased - exitStart) / (1 - exitStart));

          return (
            <div
              key={`letter-${i}`}
              className="pointer-events-none absolute z-[12] flex size-10 items-center justify-center will-change-transform"
              style={{
                left: pos.x,
                top: pos.y,
                transform: `translate(-50%, -50%) scale(${scale}) rotate(${rotate}deg)`,
                opacity:
                  (eased < 0.03 ? 0 : 0.5 + eased * 0.28) * arriveFade,
              }}
              aria-hidden
            >
              <div className="flex size-8 items-center justify-center rounded-lg border border-jade/30 bg-card/95 warm-shadow-md shadow-[0_4px_16px_-4px_rgba(48,164,108,0.22)] backdrop-blur-[2px]">
